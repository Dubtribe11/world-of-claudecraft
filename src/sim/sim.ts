import {
  ABILITIES, ARENA_SLOT_COUNT, BG_SLOT_COUNT, CAMPS, CLASSES, DUNGEONS, DUNGEON_LIST, DungeonDef,
  arenaOrigin, battlegroundOrigin, dungeonAt,
...
export class Sim {
  ...
  addPlayer(cls: PlayerClass, name: string, opts?: { autoEquip?: boolean; state?: CharacterState }): number {
    ...
    const meta: PlayerMeta = {
      ...
      squadRating: opts?.state?.squadRating ?? ARENA_BASE_RATING,
  ...
  isHostileTo(attacker: Entity, target: Entity): boolean {
    ...
    if (target.kind === 'player' && attacker.kind === 'player') {
      ...
      if (match && match.state === 'active' && (match.a === target.id || match.b === target.id)) return true;
  ...
  bgQueueJoin(pid?: number): void {
    ...
    if (this.arenaMatches.has(id) || this.arenaQueue.includes(id)) { this.error(id, 'Leave the arena queue first.'); return; }
    ...
    const ratingAvg = members.reduce((s, m) => s + (this.players.get(m)?.squadRating ?? ARENA_BASE_RATING), 0) / members.length;
  ...
  private startBgMatch(teamA: number[], teamB: number[]): void {
    ...
    if (slot === null) { this.bgQueue.unshift({ pids: [...teamA, ...teamB], ratingAvg: ARENA_BASE_RATING }); return; }
  ...
  private bgTeamAvg(pids: number[]): number {
    ...
    return pids.reduce((s, p) => s + (this.players.get(p)?.squadRating ?? ARENA_BASE_RATING), 0) / pids.length;
  }
  ...
  private endBgMatch(match: BgMatch, winnerTeam: Team | null, _reason: 'caps' | 'timeout' | 'forfeit'): void {
    ...
    for (const team of [0, 1] as Team[]) {
      ...
      for (const pid of match.teams[team]) {
        ...
        meta.squadRating = Math.max(ARENA_MIN_RATING, before + delta);
  ...
  squadLadder(): import('../world_api').ArenaLadderEntry[] {
    const rows: import('../world_api').ArenaLadderEntry[] = [];
    for (const meta of this.players.values()) {
