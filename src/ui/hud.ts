...
export class Hud {
  ...
  private renderBgWindow(): void {
    ...
    const allTime = this.bgAllTime && this.bgAllTime.length > 0 ? `<div class="arena-sub">Ladder — All-Time</div>${allTimeRows}` : '';
    ...
    el.innerHTML = `<div class="panel-title"><span>Ravenrift <span style="color:#998d6a;font-size:11px">5v5 Capture the Flag</span></span><span class="x-btn" data-close>✕</span></div>`
      ...
      + action
      + `<div class="arena-sub">Ladder — Online</div>${online}${allTime}`;
    el.querySelector('[data-close]')?.addEventListener('click', () => { el.style.display = 'none'; });
  ...
  closeAll(): boolean {
    ...
    for (const id of ['#quest-dialog', '#loot-window', '#vendor-window', '#bags', '#char-window', '#spellbook', '#quest-log-window', '#map-window', '#arena-window', '#bg-window']) {
      const el = $(id);
